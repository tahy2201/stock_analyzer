...
class MarketAnalyzer:
    ...
    def calculate_market_trend(self, index_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # 価格変化率
            price_changes = {
                '1d': ((close_prices.iloc[-1] / close_prices.iloc[-2]) - 1) * 100 if len(close_prices) > 1 else 0,
                '5d': ((close_prices.iloc[-1] / close_prices.iloc[-6]) - 1) * 100 if len(close_prices) > 5 else 0,
                '1m': ((close_prices.iloc[-1] / close_prices.iloc[-21]) - 1) * 100 if len(close_prices) > 21 else 0,
                '3m': ((close_prices.iloc[-1] / close_prices.iloc[-63]) - 1) * 100 if len(close_prices) > 63 else 0
            }
    ...
    def is_market_overheated(self, threshold_days: int = 30, threshold_percentage: float = 15.0) -> Dict:
        """
        市場が高騰状態かどうかを判定
        """
        market_status = {}
        ...
        for index_name in self.market_indices.keys():
            try:
                ...
                market_status[index_name] = {
                    ...
                    'reason': f"{threshold_days}日間で{change_percentage:.1f}%変化" if is_overheated else "正常範囲"
                }
    ...
    def get_market_summary(self) -> Dict:
        ...
        summary['investment_timing'] = self._determine_investment_timing(
            ...
            overheated_analysis['overall_overheated']
        )
...
if __name__ == "__main__":
    ...
    print(f"  過熱状況: {'過熱' if summary.get('overheated_analysis', {}).get('overall_overheated') else '正常'}")
