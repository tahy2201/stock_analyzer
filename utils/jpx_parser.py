...
class JPXParser:
    ...
    def download_jpx_file(self, url: str = None) -> bool:
        """
    ...
    def parse_jpx_excel(self, file_path: Path = None) -> List[Dict]:
        """
    ...
    def _extract_company_info(self, row) -> Optional[Dict]:
        """
        行データから企業情報を抽出
    ...
    def _determine_enterprise_status(self, name: str, sector: str, market: str) -> bool:
        """
        ...
        # 市場区分による判定
        if market:
            if any(premium_market in market for premium_market in ['プライム', 'Prime', '東証1部', '1部']):
                return True
            elif any(standard_market in market for standard_market in ['スタンダード', 'Standard', '東証2部', '2部']):
                return True
    ...
    def update_jpx_data(self) -> bool:
        ...
        if not self.jpx_file_path.exists():
            ...
            logger.info("https://www.jpx.co.jp/markets/statistics-equities/misc/01.html から手動ダウンロードしてください")
            return False
