...
# ページ設定
st.set_page_config(
    page_title="株式分析システム", page_icon="📈", layout="wide", initial_sidebar_state="expanded"
)
...
def show_dashboard():
    ...
    with col4:
        success_rate = (
            stats.get("symbols_with_prices", 0) / max(stats.get("companies_count", 1), 1)
        ) * 100
    ...
    if not candidates.empty:
        ...
        if not filtered.empty:
            ...
            (
                ...
                header_col7,
                header_col8,
            ) = st.columns([1.2, 2.5, 1.3, 1.2, 1, 1.2, 1.2, 0.8])
            ...
            for idx, row in filtered.iterrows():
                ...
                with col2:
                    st.markdown(
                        f'<div class="cell-text">{row["name"]}</div>', unsafe_allow_html=True
                    )

                with col3:
                    st.markdown(
                        f'<div class="cell-text">{row["sector"] if pd.notna(row["sector"]) else "N/A"}</div>',
                        unsafe_allow_html=True,
                    )
                ...
                with col4:
                    if pd.notna(row["latest_price"]):
                        st.markdown(
                            f'<div class="cell-text">{row["latest_price"]:.0f}円</div>',
                            unsafe_allow_html=True,
                        )
                    else:
                ...
                with col5:
                    div_color = (
                        "🔴"
                        if row["divergence_rate"] < -5
                        else "🟡"
                        if row["divergence_rate"] < -1
                        else "🟢"
                    )
                    st.markdown(
                        f'<div class="cell-text">{div_color} {row["divergence_rate"]:.1f}%</div>',
                        unsafe_allow_html=True,
                    )
                ...
                with col6:
                    div_yield_color = (
                        "🟢"
                        if row["dividend_yield"] > 5
                        else "🟡"
                        if row["dividend_yield"] > 2
                        else "⚪"
                    )
                    st.markdown(
                        f'<div class="cell-text">{div_yield_color} {row["dividend_yield"]:.1f}%</div>',
                        unsafe_allow_html=True,
                    )
                ...
                with col7:
                    if pd.notna(row["ma_25"]):
                        st.markdown(
                            f'<div class="cell-text">{row["ma_25"]:.0f}円</div>',
                            unsafe_allow_html=True,
                        )
                    else:
                        st.markdown(f'<div class="cell-text">N/A</div>', unsafe_allow_html=True)
                ...
                with col8:
                    ...
                    st.markdown(
                        f'<div class="cell-text">{score_color} {row["score"]}</div>',
                        unsafe_allow_html=True,
                    )
        ...
        if company_info:
            ...
            if not price_data.empty:
                ...
                if not trading_data.empty:
                    ...
                    trading_data["date_str"] = pd.to_datetime(trading_data["date"]).dt.strftime(
                        "%Y-%m-%d"
                    )
