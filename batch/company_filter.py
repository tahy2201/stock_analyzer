...
class CompanyFilter:
    ...
    def is_enterprise_company(self, company_info: Dict) -> Tuple[bool, str]:
        ...
        # 除外業種
        excluded_sectors = [
            'real estate', 'reits', 'utilities', 'mutual funds',
            '不動産', 'リート', 'ファンド', '投資'
        ]
    ...
    def update_company_enterprise_status(self, symbols: List[str] = None) -> Dict[str, Dict]:
        ...
        logger.info(f"企業情報更新完了: 成功 {success_count}/{len(symbols)}, エンタープライズ {enterprise_count}")
    ...
    def get_filtering_stats(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'enterprise_ratio': len(enterprise_companies) / len(all_companies) if all_companies else 0,
                'employee_distribution': employee_stats,
                'revenue_distribution': revenue_stats,
                'sector_distribution': dict(sorted(sector_stats.items(), key=lambda x: x[1], reverse=True))
            }
