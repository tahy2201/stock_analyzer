...
class StockDataCollector:
    ...
    def fetch_stock_data_with_retry(
        self, symbol: str, period_days: int = DATA_DAYS, max_retries: int = 3
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                hist = ticker.history(
                    start=start_date, end=end_date, interval="1d", auto_adjust=True, prepost=False
                )
                ...
                if attempt == max_retries:
                    ...
                    raise Exception(
                        f"株価データ取得に失敗 (最大{max_retries + 1}回試行): {symbol} - レート制限により処理を中断"
                    )
    ...
    def get_last_trading_day(self, from_date: datetime | None = None) -> datetime:
        ...
        for _ in range(10):  # 最大10日遡る
            # 平日かつ祝日でない場合は取引日
            if (
                check_date.weekday() < 5  # 月-金（0-4）
                and not any(holiday.date() == check_date.date() for holiday in all_holidays)
            ):
                return check_date
    ...
    def is_data_up_to_date(self, symbol: str, max_days_old: int = 1) -> bool:
        ...
        try:
            ...
            if is_recent:
                logger.debug(
                    f"{symbol}: 最新データ有り ({latest_date.date()}, 最終取引日: {last_trading_day.date()})"
                )
            else:
                days_behind = (last_trading_day.date() - latest_date.date()).days
                logger.debug(
                    f"{symbol}: データ更新が必要 ({latest_date.date()}, {days_behind}取引日遅れ)"
                )
    ...
    def collect_single_stock(
        self, symbol: str, include_dividend: bool = True, force_update: bool = False
    ) -> bool:
        """
        単一銘柄のデータ収集とデータベース保存
        """
    ...
    def collect_batch_stocks(self, symbols: List[str], max_workers: int = 5) -> Dict[str, bool]:
        ...
        for i in range(0, len(symbols), BATCH_SIZE):
            ...
            logger.info(
                f"バッチ処理中: {i + 1}-{min(i + BATCH_SIZE, len(symbols))} / {len(symbols)}"
            )
    ...
    def update_all_stocks(
        self, enterprise_only: bool = True, force_update: bool = False, markets: List[str] = []
    ) -> Dict[str, int]:
        """
        全銘柄のデータ更新
        """
        ...
        companies = self.db_manager.get_companies(
            is_enterprise_only=enterprise_only, markets=markets if markets else None
        )
        if not companies:
        ...
        if not force_update:
            ...
            logger.info(
                f"最新データ有り: {up_to_date_count}銘柄, 更新必要: {len(symbols_to_update)}銘柄"
            )
        ...
        skipped_count = (
            len([company["symbol"] for company in companies]) - len(symbols)
            if not force_update
            else 0
        )

        logger.info(
            f"データ更新完了: 成功 {success_count}, 失敗 {failed_count}, スキップ {skipped_count}"
        )
