...
from config.settings import (
    MA_PERIOD, DIVERGENCE_THRESHOLD, 
...
class TechnicalAnalyzer:
    ...
    def get_dividend_yield(self, symbol: str) -> Optional[float]:
        ...
        try:
            with self.db_manager.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT dividend_yield FROM companies WHERE symbol = ?
                ''', (symbol,))
                result = cursor.fetchone()
                
                if result and result['dividend_yield'] is not None:
                    return float(result['dividend_yield'])
                else:
                    logger.debug(f"配当利回りデータなし: {symbol}")
                    return 0.0
    ...
    def analyze_batch_stocks(self, symbols: List[str], markets: List[str] = []) -> Dict[str, Optional[Dict]]:
        """
        複数銘柄の技術分析をバッチ実行
        """
            ...
            for candidate in candidates:
                try:
                    ...
                    if not price_data.empty:
                        ...
                        price_change_1d = ((price_data['close'].iloc[-1] / price_data['close'].iloc[-2]) - 1) * 100 if len(price_data) > 1 else 0
    ...
    def _analyze_ma_trend(self, ma_series: pd.Series) -> str:
        ...
        longer_slope = ma_series.iloc[-1] - ma_series.iloc[0] if len(ma_series) >= 5 else recent_slope
...
if __name__ == "__main__":
    ...
    for symbol, result in results.items():
        if result:
            print(f"  {symbol}: 乖離率 {result['divergence_rate']}%, 配当利回り {result['dividend_yield']}%")
        else:
            print(f"  {symbol}: 分析失敗")
